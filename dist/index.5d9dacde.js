...
const tomaCiudadInput = function() {
    form.addEventListener("submit", function(event) {
        ...
        const { elements: { ciudad  }  } = event.target;
...
const busquedaClima = async function(busqueda) {
    ...
    https://api.openweathermap.org/data/2.5/weather?q=${busqueda}&appid=c1e47110da4d70de2cafd30f980532f1&units=metric`);
        ...
        if (!posClima.ok) throw new Error(`${dataClima.message} (${posClima.status})`);
...
/*const insertarBusquda = function () {
   ...
   insertarBusquda(recipiente, 1677823200)*/ //API meteorology actual
...
const clima = async function(busqueda) {
    ...
    http://api.openweathermap.org/data/2.5/forecast?q=${busqueda}&appid=c1e47110da4d70de2cafd30f980532f1&units=metric`);
        ...
        const converList = listArr.map((listNuva)=>listNuva.dt_txt.split(" ")).filter((fil)=>fil.includes("00:00:00")).map((orden)=>orden.join(" "));
        const recepcionObje = [];
  ...
  <p class="card__row card-diatexto">${diaSem}</p>
            ...
            <ul class="min__max">
            <li class= "min__max--ajuste"><p class="card__row">min</p><span class="min">${Math.trunc(main.temp_min)}°</span></li>
            <hr>
            <li class= "min__max--ajuste"><p class="card__row">max</p><span class="max">${Math.trunc(main.temp_max)}°</span></li>
            </ul>
            <a href="#">more info</a>
        </div>
  `;
        ...
        const tomaFecha = function(intFecha) {
            ...
            const converFecha = listArr.map((listNuva)=>listNuva.dt_txt.split(" ")).filter((fil)=>fil.includes(intFecha)).map((orden)=>orden.join(" "));
